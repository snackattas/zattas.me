...
export interface HaikuCollection {
  author: string;
  haikus: string[];
}

export const haikuCollections: HaikuCollection[] = [
  {
    author: "Matsuo Basho",
    haikus: [
...
export function getRandomHaiku(): Haiku {
  const randomCollectionIndex = Math.floor(Math.random() * haikuCollections.length);
  const collection = haikuCollections[randomCollectionIndex];
  
  if (!collection) {
    // Fallback to first collection if something goes wrong
    const fallback = haikuCollections[0]!;
    return {
      text: fallback.haikus[0]!,
      author: fallback.author
    };
  }
  
  const randomHaikuIndex = Math.floor(Math.random() * collection.haikus.length);
  const haikuText = collection.haikus[randomHaikuIndex];
  
  if (!haikuText) {
    // Fallback to first haiku if something goes wrong
    return {
      text: collection.haikus[0]!,
